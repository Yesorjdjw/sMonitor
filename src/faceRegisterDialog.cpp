...
FaceRegisterDialog::FaceRegisterDialog(FaceEngine *engine, QWidget *parent)
    : QDialog(parent)
    ...
    , m_photoCount(0)
{
    ...
    bool camOk = false;
    for (int ci = 0; ci <= 5; ci++) { if (m_cap.open(ci)) { camOk = true; break; } }
    if (!camOk) {
        m_cap.open("/opt/aicTrain/camCapture/adver.mp4");
    }

    m_timer = new QTimer(this);
    connect(m_timer, &QTimer::timeout, this, &FaceRegisterDialog::onFrameCaptured);
    m_timer->start(50);
...
void FaceRegisterDialog::onFrameCaptured()
{
    cv::Mat frame;
    m_cap >> frame;
...
void FaceRegisterDialog::on_confirmBt_clicked()
{
    ...
    m_faceDir = "/opt/aicTrain/sMonitor_server/facedata" + QString("/Faces/s%1").arg(m_label);
...
void FaceRegisterDialog::on_captureBt_clicked()
{
    ...
    if (m_photoCount >= MAX_PHOTOS) {
        QString atPath = "/opt/aicTrain/sMonitor_server/facedata/Faces/at.txt";
        QFile af(atPath);
        ...
        QString namePath = "/opt/aicTrain/sMonitor_server/facedata/Faces/name.txt";
        QFile nf(namePath);
        ...
        QString photoPath = QString("./facedata/Faces/s%1/1.jpg").arg(m_label);

        m_engine->trainModel(atPath, "/opt/aicTrain/sMonitor_server/facedata/MyFacePCAModel.xml");
...
void FaceRegisterDialog::on_doneBt_clicked()
{
    accept();
}

void FaceRegisterDialog::on_cancelBt_clicked()
{
    reject();
}
